...
struct __attribute__ ((packed)) Vertex {
    ...
    bool operator==(const Vertex& rhs) const {
        return !std::memcmp(&x, &rhs.x, sizeof(float)) &&
               y == rhs.y &&
               z == rhs.z &&
               nx == rhs.nx &&
               ny == rhs.ny &&
...
int main(int argc, char** argv)
{
    ...
    std::cout << ":: Opening PLY with " << vertices.size() << " vertices!" << std::endl;

    std::cout << ":: Random Read Test" << std::endl;
    std::cout << "    :: Generating " << 50000000 << " random indices to query" << std::endl;
    ...
    for (std::size_t ridx = 0; ridx < rnd_indices.size(); ridx++)
    {
        if (vertices.at(ridx).x != (ridx*9) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).x << std::endl;
        }
        else if (vertices.at(ridx).y != (ridx*9 + 1) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).y << std::endl;
        }
        else if (vertices.at(ridx).z != (ridx*9 + 2) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).z << std::endl;
        }
        else if (vertices.at(ridx).nx != (ridx*9 + 3) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).nx << std::endl;
        }
        else if (vertices.at(ridx).ny != (ridx*9 + 4) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).ny << std::endl;
        }
        else if (vertices.at(ridx).nz != (ridx*9 + 5) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).nz << std::endl;
        }
        else if (vertices.at(ridx).red != (ridx*9 + 6) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).red << std::endl;
        }
        else if (vertices.at(ridx).green != (ridx*9 + 7) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).green << std::endl;
        }
        else if (vertices.at(ridx).blue != (ridx*9 + 8) % 128) {
            std::cout << "ERR " << (ridx*9) % 128 << " != " << vertices.at(ridx).blue << std::endl;
        }
    }
